...
class LightComponent(AbstractPlaceable):
    def __init__(self, name: str, light_class: str, light_component: unreal.UObject = None):
        super().__init__(name, "LightComponent")
        self.light_class: str = light_class
        self.light_component: unreal.UObject = light_component
        self.light_component_name: str = ""  # ...but we may still need its name for saving it later to json

    def get_materials(self) -> List[unreal.UObject]:
        return []

    def set_materials(self, materials: List[unreal.UObject]) -> None:
        return

    def add_material(self, material: unreal.UObject) -> None:
        return

    def remove_material(self, material: unreal.UObject = None, index: int = -1) -> None:
        return
    ...
    def draw_debug_box(self, player_controller: unreal.UObject) -> None:
    ...
    def instantiate(self) -> Tuple[AbstractPlaceable, List[AbstractPlaceable]]:
        ...
        collection_actor = find_all("StaticLightCollectionActor")[-1]
    ...
    def destroy(self) -> List[AbstractPlaceable]:
        ...
        if not self.b_dynamically_created:
            self.light_component_name = ENGINE.PathName(self.light_component)
    ...
    def store_default_values(self, default_dict: dict) -> None:
        if self.light_component and ENGINE.PathName(self.light_component) not in default_dict:
            default_dict[ENGINE.PathName(self.light_component)] = {
    ...
    def restore_default_values(self, default_dict: dict) -> None:
        if self.light_component and ENGINE.PathName(self.light_component) in default_dict:
            defaults = default_dict[ENGINE.PathName(self.light_component)]
    ...
    def save_to_json(self, saved_json: dict) -> None:
        if not self.b_dynamically_created and self.b_default_attributes and not self.is_destroyed:
            ...
            smc_list[ENGINE.PathName(self.light_component)] = {"Location": self.get_location(),
                                                                          "Rotation": self.get_rotation(),
                                                                          "Scale": self.get_scale(),
                                                                          "Scale3D": self.get_scale3d(),
                                                                          "Materials":
            ...
            lights_list.append(
                {ENGINE.PathName(self.light_class): {"Location": self.get_location(),
                                                                "Rotation": self.get_rotation(),
                                                                "Scale": self.get_scale(),
                                                                "Scale3D": self.get_scale3d(),
                                                                "Materials":
